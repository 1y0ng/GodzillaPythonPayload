# GodzillaPythonPayload

本仓库为 Godzilla提供了python内存马的有效载荷、加密器、插件等支持，基于BeichenDream大佬的GodzillaNodeJsPayload进行开发，在这里表示respect

## 项目简介

GodzillaPythonPayload是一个为 Godzilla 平台提供的插件项目（基于 Java + Maven 构建）。本仓库以 Maven 为构建工具，生成的 jar 可作为 Godzilla 客户端的插件使用。
...
```bash
# 克隆仓库（如尚未克隆）
git clone https://github.com/1y0ng/GodzillaPythonPayload.git
cd GodzillaPythonPayload
...
由于python的WEB框架较多，利用方法（eval、exec任意代码执行、ssti注入）复杂多变，目前生成的有效载荷为tornado框架的内存马类型，仅作为参考，无法直接使用，需要根据目标对应的web框架（Flask、Django）进行修改......
